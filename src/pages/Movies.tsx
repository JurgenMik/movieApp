...
function Movies() {
    ...
    const [movies, setMovies] = React.useState<movieInt[]>([]);
    ...
    const getMovies = () => {
        setMovies(movies.concat(data.filter(data => data.category === 'Movie')));
    }

    return(
        <div className="text-white col-span-7 sm:ml-0 ml-6">
           <h1 className="sm:text-3xl text-xl mt-16">Movies</h1>
            <div className="w-full grid sm:grid-cols-4 grid-cols-2">
                {movies.map((info : any, index) => {
                    return(
